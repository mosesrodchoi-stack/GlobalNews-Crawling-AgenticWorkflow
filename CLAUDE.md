...
├── README.md                              ← 프로젝트 진입점 (GlobalNews 시스템 소개)
├── GLOBALNEWS-README.md                   ← [자식] 시스템 상세 소개 + 실행 결과
├── GLOBALNEWS-ARCHITECTURE-AND-PHILOSOPHY.md  ← [자식] 설계 철학, 아키텍처 심층 분석
├── GLOBALNEWS-USER-MANUAL.md              ← [자식] 운영 가이드 (CLI, 대시보드, 자동화)
├── CLAUDE.md                              ← 이 파일 (Claude Code 전용 지시서)
├── AGENTS.md                              ← 모든 AI 에이전트 공통 지시서 (Hub — 방법론 SOT)
├── GEMINI.md                              ← Gemini CLI 전용 지시서 (Spoke)
├── AGENTICWORKFLOW-USER-MANUAL.md         ← [부모] 프레임워크 사용자 매뉴얼
...
│   │   └── run.md                         (시스템 실행 — /run, 크롤링+분석 파이프라인 실행)
│   ├── hooks/scripts/                     ← Context Preservation System + Setup Hooks + Safety Hooks
│   │   ├── context_guard.py               (Hook 통합 디스패처 — 4개 이벤트의 단일 진입점)
│   │   ├── _context_lib.py                (공유 라이브러리 — Hook 인프라 핵심, ~7,500줄. 개별 기능 상세: 아래 §Claude의 활용 방법)
│   │   ├── save_context.py                (SessionEnd/PreCompact 저장 엔진 + compact 제안 마커 정리(SessionEnd only))
│   │   ├── restore_context.py             (SessionStart 복원 — RLM 포인터 + 동적 RLM 쿼리 힌트 + Error→Resolution 자동 표면화 + 반복 에러 타입 자동 표면화(P1 Consumer) + 학습된 작업 패턴 자동 표면화(P1 Consumer) + Predictive Debugging 위험 점수 캐시 생성 + Team State 복원(active_team 표면화) + 최근 지시 표면화(스냅샷에서 "최근 지시" 추출))
...
│   │   ├── generate_context_summary.py    (Stop 증분 스냅샷 + Knowledge Archive + E5 Guard + Autopilot Decision Log 안전망 + Autopilot Stall Detection + Adversarial Review 누락 감지 + Verification 누락 감지 + Traceability 누락 감지 + DKS 누락 감지 + Diagnosis 누락 감지 + ULW Compliance 안전망 + Phase-Aware Compact 제안)
...
│   │   ├── validate_decision_log.py       (Decision Log P1 검증 독립 스크립트 — DL1-DL8 구조적 무결성(DL7-DL8 내용 품질 WARNING), JSON 출력, Hook 아님 — Orchestrator가 (human) 단계 자동 승인 후 수동 호출)
│   │   ├── validate_pacs.py               (pACS P1 검증 독립 스크립트 — PA1-PA7 구조적 무결성(PA7: RED < 50 차단) + L0 Anti-Skip Guard(L0a-L0c) + L0d 구조 검증 + L0d blocking mode(config blocking_steps), JSON 출력, Hook 아님 — Orchestrator가 수동 호출)
│   │   ├── validate_review.py             (Adversarial Review P1 검증 독립 스크립트 — R1-R5 구조적 무결성 + pACS Delta + Focus 검증(FS1-FS3), JSON 출력, Hook 아님 — Orchestrator가 수동 호출)
...
│   │   ├── validate_retry_budget.py       (Retry Budget P1 검증 독립 스크립트 — RB1-RB3 재시도 예산 판정(ULW-aware) + RP1-RP3 Circuit Breaker(진전도 감지) + 히스토리 기록, JSON 출력, Hook 아님 — Orchestrator가 재시도 전후 수동 호출)
│   │   ├── setup_init.py                  (Setup Init — 인프라 건강 검증 + SOT 쓰기 패턴 검증(P1 할루시네이션 봉쇄) + 런타임 디렉터리 자동 생성(SOT 존재 시) + 도메인 venv P1 검증(ENV1a-ENV1d: .venv 존재·Python 버전·spaCy import·모델 로드), --init 트리거)
│   │   ├── setup_maintenance.py           (Setup Maintenance — 주기적 건강 검진, --maintenance 트리거)
│   │   ├── block_destructive_commands.py  (PreToolUse Safety Hook — 위험 명령·시크릿 소스·파괴적 SQL 차단(P1 할루시네이션 봉쇄), exit code 2로 차단 + Claude 자기 수정)
│   │   ├── block_secret_leak.py           (PostToolUse Safety Hook — 시크릿 출력 감지 경고, exit code 0 경고 전용)
...
│       │   └── SKILL.md
│       ├── insight-report/               ← [자식] 통찰 보고서 생성 스킬
...
│   ├── sot_manager.py                     (SOT 원자적 읽기/쓰기/검증 — fcntl 파일 잠금, SM-ST1 완료 보호(current_step < total_steps 시 "completed" 거부), SM5 품질 게이트 증거 강제(SM5a verification-log 존재, SM5b pACS-log 존재, SM5c pACS ≥ 50, SM5d review verdict ≠ FAIL — HUMAN_STEPS 면제, --force 우회))
│   ├── workflow_starter.py                (워크플로우 시작 컨텍스트 생성기 — /start 진입점)
│   ├── validate_step_transition.py        (단계 전환 사전 조건 검증 — ST1-ST6)
│   ├── run_quality_gates.py               (품질 게이트 시퀀서 — L0→L1→L1.5→L2 순서 보장 + TM 팀 병합 검증(TEAM_STEPS) + Stall Detection 통합)
│   ├── extract_orchestrator_step_guide.py (Playbook에서 단계별 가이드 추출 — 에이전트 컨텍스트 주입)
│   ├── validate_site_coverage.py          (121개 사이트 커버리지 검증)
│   ├── validate_site_registry_sync.py    (121개 사이트 레지스트리 교차 검증 — 5개 소스 동기화)
...
├── main.py                               ← [자식] CLI 진입점 (crawl/analyze/full/status)
├── dashboard.py                          ← [자식] Streamlit 대시보드 (6개 탭)
├── .venv/                                ← Python 3.13 venv (gitignored) — 도메인 NLP 파이프라인 전용
├── requirements.txt                      ← [자식] Python 의존성 (44+ 패키지)
├── pyproject.toml                        ← [자식] 프로젝트 메타데이터 + 린터 설정 (requires-python >=3.12,<3.14)
├── src/                                  ← [자식] 핵심 소스 코드 (171개 파일, ~48,000 LOC)
│   ├── crawling/                         (121개 사이트 어댑터 + 안티블록 엔진 + DynamicBypassEngine)
│   ├── analysis/                         (8단계 NLP 파이프라인)
│   ├── storage/                          (Parquet ZSTD + SQLite FTS5/vec)
│   └── utils/                            (로깅, 설정, 에러 처리)
├── config/                               ← 설정 파일 (도메인 + 프레임워크)
│   ├── sources.yaml                      (121개 사이트 설정 — 도메인)
│   ├── pipeline.yaml                     (8단계 파이프라인 설정 — 도메인)
│   ├── output-structure.yaml             (L0d 구조 검증 설정 — 프레임워크, NOT SOT)
│   └── review-focus.yaml                 (리뷰 집중 영역 설정 — 프레임워크, NOT SOT)
├── data/                                 ← [자식] 날짜별 파티션 데이터 (gitignored)
├── research/                             ← [자식] Research Phase 산출물 (Steps 1-4)
├── planning/                             ← [자식] Planning Phase 산출물 (Steps 5-8)
...
#### Pre-L1 /simplify Quality Pass (코드 생성 단계 Step 9-17만, 비차단)
- [ ] 산출물 디스크 저장 후, L1 Verification 전에 실행
- [ ] `/simplify` 실행 → 제안 사항 적용 (기능 변경 없는 개선만)
- [ ] 비차단 — /simplify는 게이트가 아닌 품질 패스 (advisory)
- [ ] /simplify 적용 후 산출물 재저장

#### 단계 완료 후 (Verification Gate — `Verification` 필드 있는 단계만)
- [ ] 산출물 파일을 디스크에 저장
- [ ] 산출물을 각 `Verification` 기준 대비 자기 검증
- [ ] 실패 기준 있으면:
  - [ ] P1 재시도 예산+진전도 확인+소비: `python3 .claude/hooks/scripts/validate_retry_budget.py --step N --gate verification --project-dir . --check-and-increment`
    - `can_retry: false`, `reason: budget_exhausted` → 사용자 에스컬레이션 (재시도 예산 소진)
    - `can_retry: false`, `reason: circuit_breaker_open` → 사용자 에스컬레이션 (진전 없음 — 3회 연속 ≤5점 개선)
    - `can_retry: true` → **Abductive Diagnosis 수행** (아래 진단 서브섹션 참조) → 진단 기반 재실행
  - [ ] 재실행 후 결과 기록: `python3 .claude/hooks/scripts/validate_retry_budget.py --step N --gate verification --project-dir . --record-attempt --pacs-score {score}`
- [ ] 모든 기준 PASS 확인
...
- [ ] pACS RED(< 50) 시:
  - [ ] P1 재시도 예산+진전도 확인+소비: `python3 .claude/hooks/scripts/validate_retry_budget.py --step N --gate pacs --project-dir . --check-and-increment`
    - `can_retry: false`, `reason: budget_exhausted` → 사용자 에스컬레이션 (재시도 예산 소진)
    - `can_retry: false`, `reason: circuit_breaker_open` → 사용자 에스컬레이션 (진전 없음 — 3회 연속 ≤5점 개선)
    - `can_retry: true` → **Abductive Diagnosis 수행** (아래 진단 서브섹션 참조) → 진단 기반 재작업 + 재채점
  - [ ] 재작업 후 결과 기록: `python3 .claude/hooks/scripts/validate_retry_budget.py --step N --gate pacs --project-dir . --record-attempt --pacs-score {score}`
- [ ] pACS YELLOW(50-69) 시: Decision Log에 약점 차원 기록 후 진행
- [ ] P1 검증 실행: `python3 .claude/hooks/scripts/validate_pacs.py --step N --check-l0 --project-dir .` (L0a-L0c + L0d 구조 검증)
- [ ] P1 검증 결과 `valid: true` 확인 (PA1-PA7 + L0 모두 통과)
- [ ] L0d WARNING 있으면: 산출물 구조 재확인 (비차단 — 캘리브레이션 중)
- [ ] SOT `outputs`에 산출물 경로 기록
- [ ] SOT `current_step` +1 증가
- [ ] `(human)` 단계: `autopilot-logs/step-N-decision.md` 생성
- [ ] `(human)` 단계: P1 결정 로그 검증 실행: `python3 .claude/hooks/scripts/validate_decision_log.py --step N --project-dir .`
- [ ] `(human)` 단계: P1 검증 결과 `valid: true` 확인 (DL1-DL6 모두 통과, DL7-DL8 WARNING 확인)
- [ ] `(human)` 단계: HQ4 이전 단계 품질 게이트 증거 확인 (직전 non-human 단계의 verification-logs + pacs-logs 존재)
...
- [ ] Verdict 확인:
  ...
  - [ ] FAIL → P1 재시도 예산+진전도 확인+소비: `python3 .claude/hooks/scripts/validate_retry_budget.py --step N --gate review --project-dir . --check-and-increment`
    - `can_retry: false`, `reason: budget_exhausted` → 사용자 에스컬레이션 (재시도 예산 소진)
    - `can_retry: false`, `reason: circuit_breaker_open` → 사용자 에스컬레이션 (진전 없음 — 3회 연속 ≤5점 개선)
    - `can_retry: true` → **Abductive Diagnosis 수행** (아래 진단 서브섹션 참조) → 진단 기반 재작업
  - [ ] 재작업 후 결과 기록: `python3 .claude/hooks/scripts/validate_retry_budget.py --step N --gate review --project-dir . --record-attempt --pacs-score {score}`
...
> Circuit Breaker(점수 정체 감지)와 FP3(동일 가설 반복 감지)는 보완적 안전망이다. Circuit Breaker는 Diagnosis **전**에 판정하고, FP3는 Diagnosis **중**에 판정한다.
...
**핵심 금지 사항 — Anti-Rationalization 테이블**:

| 금지 사항 | 흔한 합리화 (Anti-Rationalization) | 왜 안 되는가 |
|----------|--------------------------------|-------------|
| `current_step`을 2 이상 한 번에 증가 | "두 단계가 사실상 하나의 작업" | 각 단계에는 독립적 Verification Gate가 있다. 건너뛰면 L1 검증 누락 |
| 산출물 없이 다음 단계 진행 | "이전 단계가 포함" | L0 Anti-Skip Guard가 파일 존재를 검증. P1 코드가 차단 |
| "자동이니까 간략하게" | "Autopilot이니까 효율성 우선" | 절대 기준 1이 최상위. Autopilot은 HOW(자동화)이지 품질 타협이 아님 |
| Verification FAIL인 채로 진행 | "사소한 기준이라 무시" | 기준은 설계 시 의도적으로 포함. 최대 10회(ULW 활성 시 15회) 재시도 후 사용자 에스컬레이션 |
| pACS 점수를 전부 90+ 부여 | "정말 잘 했으니 당연" | Pre-mortem에서 약점 못 찾으면 자기기만. pACS는 약점 인식이 전제 |
| Review 이슈 0건으로 PASS 처리 | "완벽한 산출물" | 완벽한 산출물은 없다. P1(R5)이 자동 거부 |
| 진단 없이 동일 접근법으로 재시도 | "이번에는 될 것" | 같은 접근법 = 같은 실패. Abductive Diagnosis 필수 |
| Circuit breaker OPEN인데 재시도 | "한 번 더 하면 나아질 것" | 3회 연속 ≤5점 개선 = 구조적 문제. 다른 접근법이나 사용자 개입 필요 |
| 코드 단계(Step 9-17)에서 /simplify 생략 | "코드가 충분히 깨끗" | 기계적 이슈 축적 → L2 리뷰 부담 증가, 전체 품질 저하 |
| Decision Log에 5단어 수준 Rationale 기록 | "형식만 맞추면 충분" | DL8(15단어 최소)이 경고. 품질 기준·이전 단계 참조(DL7) 없는 Rationale은 감사 추적 무의미 |
