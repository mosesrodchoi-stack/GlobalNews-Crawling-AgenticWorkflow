...
| crawling | 10 | 안티블록, 서킷브레이커, 파이프라인, 어댑터, 세션, 중복제거, **DynamicBypass** |
...
| **P1 할루시네이션 봉쇄** | 14개 결정론적 검증 스크립트 (12 `validate_*.py` + `diagnose_context.py` + SM5 gate evidence in `sot_manager.py`) + D-7 동기화 테스트 (13 instances) |
...
| 지표 | 값 |
|------|-----|
| 소스 코드 | ~47,700 LOC (src/) |
| 테스트 코드 | ~24,350 LOC (tests/) |
| 총 코드 | ~72,050 LOC |
| Python 모듈 | 171개 |
| 테스트 파일 | 54개 |
| 테스트 케이스 | ~2,565개 |
