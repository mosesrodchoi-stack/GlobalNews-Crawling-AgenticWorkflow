...
**P1 검증 (Decision Log 구조 무결성):**

```bash
python3 .claude/hooks/scripts/validate_decision_log.py --step N --project-dir .
```

DL1-DL6 6항목을 결정론적으로 검증합니다: 파일 존재(DL1), 최소 크기 100 bytes(DL2), 필수 섹션 5개(DL3: Step, Checkpoint Type, Decision, Rationale, Timestamp), 단계 번호 일치(DL4), Rationale ≥10자(DL5), Decision ≥5자(DL6).
...
- `(human)` 단계에서 HQ 품질 게이트(Decision Log 존재 + SOT 기록 + 직전 산출물 검증) 자동 실행
...
| 단계 전환 시 | `validate_step_transition.py` ST7 | Autopilot `(human)` 단계에서 Decision Log 존재 필수 |
| `(human)` 단계 완료 후 | `run_quality_gates.py` HQ1-HQ3 | Decision Log P1 검증(HQ1) + SOT 기록 확인(HQ2) + 직전 산출물 L0 검증(HQ3) |
